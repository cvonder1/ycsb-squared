...
public class InMemoryIdStore implements IdStore {
    ...
    public void store(CollectionName collectionName, long id) {
        ...
        try {
            ...
            if(pos > Integer.MAX_VALUE) {
                throw new UnsupportedOperationException("ids are only supported up to " + Integer.MAX_VALUE);
            }
            if (pos >= list.length) {
    ...
    private synchronized Pair<ReadWriteLock, ArrayWrapper> registerNewCollection(CollectionName collectionName) {
        var lockAndList = collections.get(collectionName);
        if(lockAndList == null) {
            lockAndList = new Pair<>(
                    new ReentrantReadWriteLock(),
                    new ArrayWrapper(new byte[0])
            );
